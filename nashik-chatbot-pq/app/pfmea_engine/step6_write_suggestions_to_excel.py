...
# "AI Context Source" (which reference the Severity table text came from)
# used to be its own column here, but with Severity always pinned to the
# hardcoded table (handbook retrieval is disabled for scoring - see
# run_pipeline.py's comment on severity_table_text), that value is the
# exact same static string on every single row of every run, so the column
# added a full-width repeated sentence with zero per-row information.
# Dropped from the sheet for now; still computed and passed through to the
# frontend/API response as ai_context_source if that ever becomes useful
# again (e.g. once retrieval comes back for a Q&A feature).
