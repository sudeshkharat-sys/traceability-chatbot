...
To compare against a different Azure-deployed model, set LLM_MODEL_PROFILE:
    ...
    gpt-5.4-mini lives on its own separate Azure resource, so it's not a
    named profile here - use the generic mechanism below with
    LLM_MODEL_PROFILE=gpt54mini and its own AZURE_GPT54MINI_* env vars.
...
8 (Moderately high) | 100% of production run affected may have to be scrapped. Failure may result in in-plant regulatory noncompliance or may have a chronic health and/or safety risk for the manufacturing or assembly worker | Line shutdown greater than full production shift; stop shipment possible; field repair or replacement required (Assembly to End User) other than for regulatory noncompliance. Failure may result in in-plant regulatory noncompliance or may have a chronic health and/or safety risk for the manufacturing or assembly worker | Loss of primary vehicle function necessary for normal driving during expected service life.
...
DETECTION_TABLE_TEXT = """Table C2.4 - PFMEA DETECTION (D), from the AIAG-VDA FMEA Handbook (1st Edition, 2019), page 203-204, verbatim.
"Detection Potential (D) for the Validation of the Process Design" - Detection Controls rated according to the
Detection Method Maturity and Opportunity for Detection. Detection is about the CURRENT Detection Control's own
reliability at catching this Failure Mode/Cause - NOT about how bad the failure is (Severity) or how often it
happens (Occurrence). Lower D is better (harder to miss); higher D is worse.
...
7 | Low | (same maturity as D=8: not proven effective/reliable) | Machine-based detection (automated or semi-automated with notification by light, buzzer, etc.), or use of inspection equipment such as a coordinate measuring machine, that SHOULD detect failure mode or failure cause.
...
5 | Moderate | (same maturity as D=6: proven effective/reliable) | Machine-based detection (semi-automated with notification by light, buzzer, etc.), or use of inspection equipment such as a coordinate measuring machine, that WILL detect failure mode or failure cause (including product sample checks).
4 | High | System has been proven to be effective and reliable (e.g. plant has experience with method on identical process or this application), gauge R&R results are acceptable, etc. | Machine-based automated detection method that will detect the failure mode DOWNSTREAM, prevent further processing, or the system will identify the product as discrepant and allow it to automatically move forward in the process until the designated reject unload area - a robust system that prevents outflow of the discrepant product from the facility.
3 | High | (same maturity as D=4: proven effective/reliable) | Machine-based automated detection method that will detect the failure mode IN-STATION, prevent further processing, or the system will identify the product as discrepant and allow it to automatically move forward in the process until the designated reject unload area - a robust system that prevents outflow of the discrepant product from the facility.
2 | Very high | Detection method has been proven to be effective and reliable (e.g. plant has experience with method, error-proofing verifications, etc.). | Machine-based detection method that will detect the CAUSE and PREVENT the failure mode (discrepant part) from being produced.
1 | Very high | Failure mode cannot be physically produced as-designed or processed, OR detection method proven to always detect the failure mode or failure cause. | (same row - error-proofing/impossibility)."""
...
- FALLBACK WHEN END USER IS BLANK (apply BEFORE the decision tree below): if the End User effect is blank/"(none recorded)"/"Nil" - meaning this failure is caught before it ever reaches the customer - do NOT default to Step 5 / Severity 1 just because there is nothing to read in that section. A failure with no End User effect can still be a real, substantial in-plant problem (line stoppage, scrap, rework). In this case, base the score on the "Your Plant effect" / "Ship to Plant effect" text instead, using the SAME table (its Impact-to-Your-Plant / Impact-to-Ship-to-Plant columns), and walk the SAME decision tree below against that text. Only score Severity 1 ("no discernible effect") if the stated Plant/Ship-to-Plant effect ITSELF describes no real consequence either - not merely because End User happened to be blank.
...
5. COSMETIC/NOISE CHECK (only if steps 1-4 are "no"): Rate 2-4 strictly by how widely customers would notice the defect, per the table (4 = very objectionable, 3 = moderately objectionable, 2 = slightly objectionable). Score 1 ONLY when the effect text states or implies literally nothing happens - do NOT score 1 just because the stated effect sounds small or minor. Any effect text that describes a real, if small, consequence (e.g. "difficulty in bolt removal for rework", "minor adjustment needed", any stated inconvenience to the process/operator) is NEVER "no discernible effect" - that is squarely S2 ("slight inconvenience to process, operation, or operator"), not S1. Confusing "minor" with "none" is a common mistake here - if you can point to any concrete stated consequence at all, the floor is S2, never S1.
...
CAUSE/MODE MISMATCH OVERRIDE FOR RECOMMENDATIONS (apply BEFORE writing either recommendation below): if you set "cause_mode_mismatch" to true above, the stated Failure Cause does NOT actually explain this row's Failure Mode - so a Prevention/Detection action built around that stated Cause would be fixing the WRONG problem (e.g. a Cause about "wrong part selected" doesn't help prevent a "scratch"). In that case, do NOT base recommended_action/detection_recommendation on the stated Cause text at all. Instead, reason about the most plausible REAL mechanism that actually produces the stated Failure MODE (e.g. a scratch during assembly is typically caused by rough handling/contact with a hard surface or another part during storage/installation, not by picking the wrong part) and recommend an action that addresses THAT mechanism, and say so explicitly at the start of the recommendation (e.g. "The plant's recorded Cause does not match this Mode; addressing the more plausible mechanism instead: ..."). If cause_mode_mismatch is false, proceed normally using the stated Cause.
...
Keep the recommendation to ONE sentence, specific enough that someone on the line could actually implement it this quarter - name the specific part/step/mechanism from the Failure Cause above (or, per the CAUSE/MODE MISMATCH OVERRIDE above, the actual plausible mechanism for the Mode when the stated Cause doesn't apply), not a generic engineering platitude.
...
Also score Detection (1-10), a SEPARATE question from Severity/the recommendation above: using ONLY the AIAG-VDA DETECTION SCORING TABLE given earlier (Table C2.4) and the Current Detection Controls (DC) text given below, determine how reliably the EXISTING DC (as currently described, not any upgrade you might recommend) would catch THIS Failure Mode before the part ships further down the line or leaves the plant. The table scores on TWO axes together - match BOTH:
- Detection Method Maturity: is the described method proven effective/reliable at this plant (e.g. "plant has experience with it", gauge R&R acceptable), or NOT proven (little/no experience, marginal results, or simply not stated as proven)? Two methods of the same physical type can land on different D scores purely because one is proven and the other isn't (e.g. human inspection: proven -> D6, not proven -> D8).
...
def _load_dotenv_into_environ():
    ...
    the real shell environment.

    This copy of the file lives at nashik-chatbot-pq/app/pfmea_engine/ (see
    the standalone pfmea_ai/ folder for the original, kept there for CLI
    testing) - so the real nashik-chatbot-pq/.env is three parents up from
    here, not two. Also checks for a dedicated .env directly alongside this
    file first, same as the original, in case one is ever added here."""
    candidates = [
