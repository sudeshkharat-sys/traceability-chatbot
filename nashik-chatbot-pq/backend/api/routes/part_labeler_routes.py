...
async def warranty_lookup(
    ...
    batteryMotor: Optional[list[str]] = Query(None),
):
    try:
        result = get_service().get_source_data(userId, partName, month, baseModel, misBucket, mfgQtr, dataSource, buyoffStage, onlineOffline, defectType, batteryMotor)
        return result
...
async def get_dashboard_data(
    ...
    try:
        return get_service().get_dashboard_data_for_source(userId, partName, month, baseModel, misBucket, mfgQtr, dataSource, buyoffStage, onlineOffline, defectType, batteryMotor)
    except Exception as e:
