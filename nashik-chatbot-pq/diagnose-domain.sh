...
echo "---"
nslookup $DOMAIN || dig $DOMAIN +short || echo "DNS lookup failed - domain may not be configured"
echo ""
...
curl -I http://localhost 2>&1 | head -5 || echo "HTTP port 80 not accessible"
echo ""
echo "Testing HTTPS (port 443):"
curl -I https://localhost --insecure 2>&1 | head -5 || echo "HTTPS port 443 not accessible"
echo ""

echo "Step 6: Checking nginx configuration..."
echo "---"
docker exec traceability-nginx nginx -t 2>&1 || echo "Cannot check nginx config"
echo ""
