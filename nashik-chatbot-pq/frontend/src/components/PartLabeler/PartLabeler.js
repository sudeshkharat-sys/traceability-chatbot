...
function PartLabeler() {
  ...
  const fetchDashboardData = async (partNameArg = null) => {
    if (!userId) return;
    try {
      const params = new URLSearchParams();
      params.append('userId', userId);
      
      // If specific part provided, use it. 
      // Else if no part provided, use all mapped labels for this image.
      if (partNameArg) {
        params.append('partName', partNameArg);
      } else if (labels && labels.length > 0) {
        labels.forEach(l => params.append('partName', l.partName));
      } else {
        // No labels mapped yet, dashboard will show 0s
      }
  ...
  const fetchActivePartHistory = async (label) => {
    ...
    try {
      ...
      console.error("No warranty history found", err);
    }
  };

  const updateAllLabelFailures = async (currentLabels, month, model, mis, qtr) => {
    if (!userId) return;
    const counts = {};
  ...
  const confirmMappingAndProcess = async (userId) => {
    ...
    try {
      const res = await fetch(`${API_BASE}/warranty-confirm-mapping`, {
        ...
        body: JSON.stringify({ tempFilePath, mapping: columnMapping, userId })
      });
      const data = await res.json();
      if (data.success) {
  ...
  const fetchFilterOptions = async () => {
    if (!userId) return;
    try {
      const res = await fetch(`${API_BASE}/filter-options?userId=${userId}`);
      const data = await res.json();
            ...
            <motion.div initial={{ opacity: 0, scale: 0.9, y: 20 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.9, y: 20 }} className="custom-modal-card">
              <div className="modal-header">
                <h3>
                  ...
                  {modalType === 'ingest-start' && 'Ingest Data'}
              ...
              <div className="modal-body">
                ...
                {modalType === 'ingest-mapping' && (
                  <div className="ingest-mapping-container">
                    <div className="ingest-notice">
                      ...
                      <p>Map your file columns. <strong>9 Required fields (*) must be set.</strong></p>
                    </div>
                    <div className="mapping-scroll-table">
                      <table className="mapping-table">
                        <thead><tr><th>Internal Field</th><th>Excel Column</th></tr></thead>
                        <tbody>
                          {['Required', 'Technical', 'Geography', 'Classification', 'Claim Info', 'Dealer', 'Specs'].map(group => (
                            <React.Fragment key={group}>
                              <tr className="group-header-row"><td colSpan="2">{group} Fields</td></tr>
                              {targetColumns.filter(tc => tc.group === group).map(col => (
                                <tr key={col.key}>
                                  <td className={col.mandatory ? 'mandatory-cell' : ''}>{col.label} {col.mandatory && <span className="req">*</span>}</td>
                                  <td>
                ...
                {modalType === 'ingest-success' && (
                  <div className="ingest-success-view">
                    ...
                    <p>Successfully processed and loaded <strong>{ingestResult}</strong> warranty records.</p>
                  </div>
                )}
              </div>
      ...
      <div className="part-labeler-content">
        <aside className="part-labeler-sidebar">
          ...
          <div className="sidebar-section">
            ...
            <button className="sidebar-btn secondary" onClick={handleDataIngestionStart}><Database size={18} /><span>Ingest Warranty Data</span></button>
            ...
            {selectedImage && (
              <div className="dashboard-analysis-section">
                <div className="dashboard-grid">
                  <CustomBarChart title="Vehicle Mfg Month Wise Data" data={dashboardData.mfgMonth} color="#f6ad55" icon={History} />
                  <CustomBarChart title="Reporting Month Wise Data" data={dashboardData.reportingMonth} color="#f6ad55" icon={FileSpreadsheet} />
                  <CustomBarChart title="Kms Wise Data" data={dashboardData.kms} color="#f6ad55" icon={Activity} />
                  <div className="dashboard-chart-card">
                    <div className="chart-header"><MapIcon size={16} /><span>Locationwise Distribution</span></div>
