...
- HARD RULE, no exceptions: if the End User effect text contains an EXPLICIT safety-consequence phrase - "increased risk of accident", "injury", "collision", any wording that directly names an accident/injury/unsafe-operation outcome - you MUST treat that phrase as applying to THIS failure and go to Step 1 below. Do NOT reason about whether that phrase is "representative" or "typical" of this specific cause, do NOT discount it as boilerplate, and do NOT skip past it to a calmer-sounding phrase elsewhere in the same list. An explicit safety phrase is never optional context - it is a direct statement of the effect and must be used.
