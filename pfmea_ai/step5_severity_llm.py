...
- STRICT ELIGIBILITY FILTER (apply this BEFORE treating anything as ambiguous - this is the main source of false ambiguity, so do not skip it): an End User effect phrase is only a valid candidate for "possible_severities" if it follows DIRECTLY from the stated Failure Mode/Cause's physical mechanism, per the existing "prefer the symptom that follows most directly" rule above. An effect that would require an ADDITIONAL, unstated failure step to occur (e.g. "LED/BULB burnout" or "water ingress" as a consequence of loose screws/vibration - both require an extra, non-stated mechanism such as prolonged unaddressed vibration or a separately-cracked seal) is NEVER a valid ambiguity candidate, no matter how it reads in isolation - discard it before doing the ambiguity check, exactly as you would if you were picking a single representative effect with no ambiguity rule in play at all. Only after discarding every indirect/extra-step effect should you check whether more than one of the REMAINING direct candidates plausibly applies.
- Genuine ambiguity, eligible for "possible_severities", exists ONLY when two or more of the remaining DIRECT candidates are each independently and equally representative of the stated Mode/Cause's own mechanism (e.g. a merged Mode like "Scratch/Damage" where "scratch" directly implies a cosmetic-only reading AND "damage" directly implies a functional reading, with no additional unstated step needed for either). If, after the filter above, only ONE direct candidate remains, that is NOT ambiguous - score it normally and set "possible_severities" to null, even if the Effect list happens to contain other unrelated phrases.
- If step 4 above found genuine ambiguity under this stricter definition, populate "possible_severities" with one entry per distinct plausible DIRECT reading, each with its own severity/definition/reasoning, sorted worst (highest severity) first.
- If there is no genuine ambiguity (one reading is clearly the most representative per the rules above, or only one candidate survives the eligibility filter), set "possible_severities" to null - do not manufacture options that aren't real.
